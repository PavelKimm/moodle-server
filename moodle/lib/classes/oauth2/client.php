...
class client extends \oauth2_client {
	...
	$isvk = ($this->get_issuer()->get_endpoint_url('token') == 'https://oauth.vk.com/access_token');
	if ($isvk){
    	    $access_t = $this->get_accesstoken()->token;
	    $url = $url.'?access_token='.$access_t.'&fields=email&v=5.103';
	}
    	$response = $this->get($url);
	//throw new moodle_exception('url='.$url.' response='.$response);
	...
	if (!$response) {
        ...
        try {
	    if ($isvk) {
		$userinfo = json_decode($response)->response[0];
		$userinfo->email = $this->get_useremail();
	    } else {
                $userinfo = json_decode($response);
            }
           // throw new moodle_exception(var_dump($userinfo));
	} catch (\Exception $e) {
            return false;
        }
