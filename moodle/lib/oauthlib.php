...
abstract class oauth2_client extends curl {
     ...
     */
    public function is_logged_in($isvk, $client=null) {
        ...
        if ($code && !in_array($code, self::$upgradedcodes) && $this->upgrade_token($code, $isvk, $client)) {
	    return true;
        }
	...
	if ($this->get_issuer()->get_endpoint_url('token') == 'https://oauth.vk.com/access_token') {
	    $params['scope'] = 'email,photo';
	}
     ...
     */
    public function upgrade_token($code, $isvk=false, $client) {
        $callbackurl = self::callback_url();
	...
	if ($isvk) {
            $params['client_secret'] = $client->get_clientsecret();
	    $params['client_id'] = $client->get_clientid();
        } else {
...
//	throw new moodle_exception($this->get_accesstoken()->token);
        return true;
    }
