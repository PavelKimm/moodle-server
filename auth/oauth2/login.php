...
//throw new moodle_exception('naw_client='.$client->get_clientid());//.'issuer='.$issuer);

if ($client) {
    $isvk = false;
    if ($issuer->get_endpoint_url('token') == 'https://oauth.vk.com/access_token') {
	$isvk = true;
    }

    if (!$client->is_logged_in($isvk, $client)) {
